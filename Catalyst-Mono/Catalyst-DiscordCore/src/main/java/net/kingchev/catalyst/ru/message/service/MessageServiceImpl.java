...
import net.kingchev.catalyst.ru.context.service.ContextService;
import net.kingchev.catalyst.ru.event.service.ActionsHolderService;
import net.kingchev.catalyst.ru.utils.DiscordUtils;
import net.kingchev.catalyst.ru.utils.LocaleUtils;
import net.kingchev.catalyst.ru.utils.PluralUtils;
...
public class MessageServiceImpl implements MessageService {
    ...
    public void onError(MessageChannel sourceChannel, String titleCode, String code, Object... args) {
        sendMessageSilent(sourceChannel::sendMessageEmbeds, createMessage(titleCode, code, args)
    ...
    public void onEmbedMessage(MessageChannel sourceChannel, String code, Object... args) {
        ...
        sendMessageSilent(sourceChannel::sendMessageEmbeds, builder.build());
    }
    ...
    public void onTempEmbedMessage(MessageChannel sourceChannel, int sec, String code, Object... args) {
        EmbedBuilder builder = getBaseEmbed();
        builder.setDescription(getMessage(code, args));
        sendTempMessageSilent(sourceChannel::sendMessageEmbeds, builder.build(), sec);
    }
    ...
    public void onTitledMessage(MessageChannel sourceChannel, String titleCode, String code, Object... args) {
        ...
        sendMessageSilent(sourceChannel::sendMessageEmbeds, createMessage(titleCode, code, args).build());
    }
    ...
    private void initCache(Class<?> clazz) {
        enumCache.computeIfAbsent(clazz, e -> {
            ...
            Stream.of(clazz.getEnumConstants()).forEach(c -> {
                Enum<?> entry = (Enum<?>) c;
                for (var locale : LocaleUtils.getSUPPORTED_LOCALES().entrySet()) {
